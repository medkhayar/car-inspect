...
export default async function CenterLayout({params,children}){
    ...
    const _center= await supabase.from('centers').select().eq('id',params.center_id).single()  
...
<div className="flex flex-1 justify-between w-full flex-col">
    <div className="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-6">
        <div className="md:col-span-6">
            <label className='text-gray-700 min-h-12 text-lg font-bold truncate' >{center.metadata.name}</label>
        </div>
        <div className="md:col-span-6">
    ...
    <div>
        <Link href={`/backoffice/provider/${params.provider_id}/centers/${center.id}/edit`} className="p-2 px-4 text-xs border-1 bg-[#3D84A7] hover:bg-[#47466D] rounded-md text-white font-bold"> edit center </Link>
    </div>
</div>

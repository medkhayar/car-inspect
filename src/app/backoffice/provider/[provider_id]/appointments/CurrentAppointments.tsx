...
import { getCenterApprovedAppointmentsByDate } from "@/app/global-actions"
...
export default function CurrentAppointments({initial_centers,provider_id}){
    ...
    const [loadingCurrentAppointments,setLoadingCurrentAppointments]=useState(false);
    const [selectedCenter,setSelectedCenter]=useState<any>(null);
    const [currentCenter,setCurrentCenter]=useState<any>(null); // for Select component value
    const [centers_lines_count,setCenters_lines_count]=useState(centers.map(c=>c.center_lines.map(l=>l)).flat().length)
    const [centers_list,setCenters_list]=useState<Option[]>(centers.map(c=> {return {
        label:c.metadata.name,
        value:c.id,
        image: <img className="rounded-full object-cover h-6 w-6 mr-2 shadow bg-white" src={c.metadata.logo?c.metadata.logo:center_logo_placeholder} ></img>
    }}))

    const selectCenter=(c)=>{
        setCurrentCenter(c);
        let center=centers.filter(ctr=>ctr.id==c.value)[0];
    ...
    function loadCurrents(){
        console.log("---",date)
        setLoadingCurrentAppointments(true);
        getCenterApprovedAppointmentsByDate(provider_id,date.format('YYYY-MM-DD')).then(res=>{
            console.log(res)
            setCenters(res.data);
            setCenters_lines_count(centers.map(c=>c.center_lines.map(l=>l)).flat().length)
            setCenters_list(centers.map(c=> {return {
                label:c.metadata.name,
                value:c.id,
                image: <img className="rounded-full object-cover h-6 w-6 mr-2 shadow bg-white" src={c.metadata.logo?c.metadata.logo:center_logo_placeholder} ></img>
            }}))
        ...
        .catch(err=>{
            setLoadingCurrentAppointments(false);
        })
    }
    useEffect(()=>{
        if(currentCenter){
            selectCenter(currentCenter)
        }
    },[centers])
    ...
    <div className="p-4 w-full  mx-auto">
        ...
        <LocalizationProvider dateAdapter={AdapterMoment} adapterLocale='en'>
            ...
            <div className="w-full p-4 sm:w-auto flex flex-col">
                
            <span className="py-2 text-sm text-gray-500"> Select a center</span>
                <Select isSearchable={true} selectClass={''} title="Select a center"  placeholder="" isClearable={false} containerClass={"h-10 mt-1 shadow-none "} primaryColor="neutral"
                    value={currentCenter} onChange={(v)=>selectCenter(v)}
                    options={centers_list}/>
                    
                    <span className="py-2 text-sm text-gray-500"> Select a date</span>
                    <StaticDatePicker  minDate={moment()} value={date} onChange={(e)=>setDate(e!)}  className="w-full p-4 pr-2 hidden lg:flex border-r-1 border-gray-200 search  " slots={{toolbar:DateEmptyToolBar,actionBar:DateEmptyActionBar}} />
                    <CardDatePicker value={date} onChange={(v)=>setDate(v)} closeOnSelect={true} className={'w-full flex flex-col sm:flex-row text-gray-500 lg:hidden  lg:w-1/2    lg:flex-col  '} dateFormat={'DD/MM/yyyy'} clearIcon={<></>} />
        ...
        {(!loadingCurrentAppointments && !selectedCenter) &&
                <div className="flex flex-col justify-center items-center mt-4 p-4  min-h-56">
                    <div className="flex justify-center items-center">
                        <FontAwesomeIcon icon={faBoxOpen} className="text-base  text-gray-400"/>
                        <span className="px-4 text-base text-gray-500">Please select a center.<br></br></span>
                    </div>
        ...
        <div className="p-4 w-full">
            ...
            {selectedCenter && <div  className="flex flex-col w-full relative">
                
                {  selectedCenter.center_lines.map(l=> <div key={`line-${l.name}`} className=" bg-white flex flex-col p-4 pb-2  mb-2">
                    
                        <h3 className="text-gray-600 text-sm pb-2">{l.name}</h3>
                        <hr/>
                        <div className=" flex flex-col py-4">
                        {loadingCurrentAppointments && <div role="status" className="max-w-full animate-pulse h-9 flex justify-center items-center  ">
                            <div className="h-1.5 bg-gray-200 rounded-full dark:bg-gray-700 w-22 max-w-full  mx-4"></div>
                            <div className="h-1.5 bg-gray-200 rounded-full dark:bg-gray-700 w-22 max-w-full  mx-4"></div>
                            <div className="h-1.5 bg-gray-200 rounded-full dark:bg-gray-700 w-22 max-w-full  mx-4"></div>
                        </div>}
                        
                            {!loadingCurrentAppointments && Object.entries(getAppointments(l.center_line_time_slots,l,selectedCenter)).map(([key,value])=>{
                                return <div key={`line-${key}`} className="">
                            ...
                            {!loadingCurrentAppointments &&  Object.entries(getAppointments(l.center_line_time_slots,l,selectedCenter)).length==0 && <>
...
{selectedAppointment && <SideModal title="Appointment validation" component={<AppointmentValidation onApprove={updateAppointment} onDeny={updateAppointment}  onCancel={()=>setSelectedAppointment(null)} appointment={selectedAppointment}/>} onCancel={()=>setSelectedAppointment(null)} onValidate={()=>{}} />}
    </div> 
}
...
export default function SignIn(){
    const {setSignUp,setSignIn,setUser} = useGlobalContext()
    ...
    const signIn=()=>{
        setError(undefined)
        setLoading(true)
        supabase.auth.signInWithPassword({email:signInData.username,password:signInData.password,options:{
            ...
            fetch('/auth/v1/login/',{
                ...
                body: JSON.stringify({email:signInData.username,password:signInData.password})}
                ).then(r=>{
                    setUser(user);
            })
        }
            ...
            setError("")
            setLoading(false);
            console.log("user",response) // get error i18n
        }
   ...
   <Link href='/' className="flex items-center sm:self-start text-picton-blue-600">
      ...
      <div className="mt-4">
          ...
          <input onInput={(e)=>setSignInData({...signInData,username:e.currentTarget.value})} className=" text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full appearance-none" type="email"/>
      </div>
      <div className="mt-4">
          ...
          <input onInput={(e)=>setSignInData({...signInData,password:e.currentTarget.value})} className=" text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded py-2 px-4 block w-full appearance-none" type="password"/>
      </div>
      <div className="mt-8">
          <button disabled={loading} onClick={()=>{signIn()}} className="flex flex-row justify-center disabled:bg-blue-500 items-center bg-blue-500 text-white font-bold py-2 px-4 w-full rounded hover:bg-blue-600">
            {!loading && <>Login</>}
            {loading && <>
      ...
      <div className=" mt-3 flex items-center justify-center">
        <p className="flex flex-col text-sm font-bold items-center justify-center mt-2 text-center text-md text-red-500">
            &nbsp;{error}&nbsp;
        </p>
      </div>
      <div className="flex items-center justify-center">
        <p className="flex flex-col items-center justify-center mt-6 text-center text-md text-gray-500">
